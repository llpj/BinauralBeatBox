...
public class SessionWiedergabe {
	...
	public SessionWiedergabe(Session session) {
		...
		// totalBeat = new byte[(int) playme.getSampleRate() * sampleSize *
		// session.getDuration()];
		// transition();
	}

	/**
	 * public static void playSession(int freqLinks, int freqRechts, int
	 * freqDauer)
	 * 
	 * @param static int j=1; freqLinks: linke Frequenz. Wenn 0, dann kein Ton
	 * @param freqRechts
	 *            : rechte Frequenz. Wenn 0, dann kein Ton
	 * @param freDsuer
	 *            : Frequennz Wiederholungen
	 * 
	 *            mit Hilfe von c.getMicrosecondPosition() ist es moeglich, die
	 *            aktuelle Zeit zu bestimmen
	 * 
	 */
	public void playSession(int freqLinks, int freqRechts) {
		...
		try {
			if (session != null) {
				...
				System.out
						.println("Ausgewaehlte Session in den PLayer geladen");
			} else
				System.err
						.println("SessionWiedergabe: Keine Session vorhanden.");
		} catch (Exception e) {
		...
		for (int curSeg = 0; curSeg < session.getNumerOfSegments(); curSeg++) {
			...
			// und mit session.getSegments().get(segmentNumber) kriegst du ein
			// segment

		}

		// Balance geandert (damit clip1 leiser ist als Clip2)
		FloatControl gainControl = (FloatControl) clip1
	...
	public byte[] getStereoSinusTone(int frequency1, int frequency2,
			AudioFormat af, double duration) {
	...
	private void transition() {
		...
		for (Segment s : session.getSegments()) {
			...
			// byte[] data = getStereoSinusTone(b.getFreq1_start(),
			// b.getFreq2_start(), playme, s.getDuration());
	 ...
	 * 
	 * @param volumn
	 *            : floet, Wert um wieviel die Lautstärke veringert (Negativer
	...
	private void changeVolumn(float volumn) {
		FloatControl gainControl = (FloatControl) clip1
				.getControl(FloatControl.Type.MASTER_GAIN);
		gainControl.setValue(volumn); // veringert / erhoeht die Lautsärke um x
										// Decibel
	}

	private void changeBalance(float balance, boolean c1c2) { // c1c2 bedeutet,
																// wenn true
																// dann setzen
																// lautsärke
																// von Clip1 um
		...
		FloatControl gainControl1 = (FloatControl) clip1
				.getControl(FloatControl.Type.MASTER_GAIN);
		FloatControl gainControl2 = (FloatControl) clip2
				.getControl(FloatControl.Type.MASTER_GAIN);
